%This is function is developted by Du Cheng,with the help of Alejandro Lopez, Navin Pokala,
...
% arrivalframeinds1 = find(arrivalcounts1>0);
% arrivalframes1 = [];
...
% %for ii=1:size(arrivalframes1)
%     %jitteramount = 3;
%    % jitterValuesY =  4*(rand(1)+4)*jitteramount;
%     %scatter(arrivalframes1(ii),3+jitterValuesY,50,'k','.');hold on;
% %end
% end
% plot((arrivalframes1/3), 2, 'o','MarkerSize',2, 'MarkerEdgeColor','b')
% 
% arrivalcounts2 = [0 diff(num_control1)];
% arrivalframeinds2 = find(arrivalcounts2>0);
% arrivalframes2 = [];
% for i = 1:length(arrivalframeinds2)
% arrivalframes2 = [arrivalframes2; repmat(arrivalframeinds2(i),arrivalcounts2(arrivalframeinds2(i)),1)];
% end
% plot((arrivalframes2/3), 100, 'o','MarkerSize',2, 'MarkerEdgeColor','b')
% 
...
CIlabel11 = strjoin({'Naive CI =',num2str(Chemotaxis_index1,'%.2f')});
CIlabel21 = strjoin({'Trained CI =',num2str(Chemotaxis_index2,'%.2f')});

plot(time1, perctarget1,'LineWidth',2,'Color','b','LineStyle','-')
hold on
plot(time1, perccontrol1,'LineWidth',2,'Color','b','LineStyle','--')
hold on
text(100,0.55, CIlabel11,'FontSize', 9)

plot(time2, perctarget2,'LineWidth',2,'Color','r','LineStyle','-')
hold on
plot(time2, perccontrol2,'LineWidth',2,'Color','r','LineStyle','--')
text(100,0.45, CIlabel21,'FontSize', 9)
...
arrivalcounts11 = [0 diff(num_targ1)];
arrivalframeinds11 = find(arrivalcounts11>0);
arrivalframes11 = [];
for i = 1:length(arrivalframeinds11)
arrivalframes11 = [arrivalframes11; repmat(arrivalframeinds11(i),arrivalcounts11(arrivalframeinds11(i)),1)];
end

for ii=1:(size(arrivalframes11))
    jitteramount = 0.01;
    jitterValuesY =  8*(rand(1))*jitteramount;
    scatter((arrivalframes11(ii)/3), 0.01+jitterValuesY,2, 'b', 'o');hold on;
end
% plot((arrivalframes11/3), 0.05, 'o','MarkerSize',2, 'MarkerEdgeColor','b')
% hold on

arrivalcounts12 = [0 diff(num_control1)];
arrivalframeinds12 = find(arrivalcounts12>0);
arrivalframes12 = [];
for i = 1:length(arrivalframeinds12)
arrivalframes12 = [arrivalframes12; repmat(arrivalframeinds12(i),arrivalcounts12(arrivalframeinds12(i)),1)];
end

for ii=1:(size(arrivalframes12))
    jitteramount = 0.01;
    jitterValuesY =  8*(rand(1))*jitteramount;
    scatter((arrivalframes12(ii)/3), 0.90+jitterValuesY,2, 'b', 'o');hold on;
end

% plot((arrivalframes12/3), 0.95, 'o','MarkerSize',2, 'MarkerEdgeColor','b')
% hold on

for m = 1:length(target_odor_linkedTracks1)
    all_frames = target_odor_linkedTracks1(m).Frames;
    max_frame = max(all_frames);
    all_distance = target_odor_linkedTracks1(m).odor_distance;
    last_distance = all_distance(end);
    if max_frame == 10800;
        if last_distance > min_distance_to_odor_mm & last_distance < max_distance_to_odor_mm
        last_frames1 = [last_frames1, last_distance];
        end
    end    
end

for ii=1:(numel(last_frames1))
    jitteramount = 100;
    jitterValuesX =  1*(rand(1))*jitteramount;
    scatter(3450+jitterValuesX, last_frames1(ii)/90, 8, 'k', 'x');hold on;
end
...
arrivalcounts21 = [0 diff(num_targ2)];
arrivalframeinds21 = find(arrivalcounts21>0);
arrivalframes21 = [];
for i = 1:length(arrivalframeinds21)
arrivalframes21 = [arrivalframes21; repmat(arrivalframeinds21(i),arrivalcounts21(arrivalframeinds21(i)),1)];
end

for ii=1:(size(arrivalframes21))
    jitteramount = 0.01;
    jitterValuesY =  8*(rand(1))*jitteramount;
    scatter((arrivalframes21(ii)/3), 0.01+jitterValuesY,2, 'r', 'o');hold on;
end
% plot((arrivalframes21/3), 0.05, 'o','MarkerSize',2, 'MarkerEdgeColor','r')
% hold on

arrivalcounts22 = [0 diff(num_control2)];
arrivalframeinds22 = find(arrivalcounts22>0);
arrivalframes22 = [];
for i = 1:length(arrivalframeinds22)
arrivalframes22 = [arrivalframes22; repmat(arrivalframeinds22(i),arrivalcounts22(arrivalframeinds22(i)),1)];
end

for ii=1:(size(arrivalframes22))
    jitteramount = 0.01;
    jitterValuesY =  8*(rand(1))*jitteramount;
    scatter((arrivalframes22(ii)/3), 0.90+jitterValuesY,2, 'r', 'o');hold on;
end
% plot((arrivalframes22/3), 0.95, 'o','MarkerSize',2, 'MarkerEdgeColor','r')
% hold on


for m = 1:length(target_odor_linkedTracks2)
    all_frames = target_odor_linkedTracks2(m).Frames;
    max_frame = max(all_frames);
    all_distance = target_odor_linkedTracks2(m).odor_distance;
    last_distance = all_distance(end);
    if max_frame == 10800;
        if last_distance > min_distance_to_odor_mm & last_distance < max_distance_to_odor_mm
        last_frames2 = [last_frames2, last_distance];
        end
    end    
end

for ii=1:(numel(last_frames2))
    jitteramount = 100;
    jitterValuesX =  1*(rand(1))*jitteramount;
    scatter(3450+jitterValuesX, last_frames2(ii)/90, 8, 'k', 'x');hold on;
end
...
for ii=1:(size(arrivalframes11))
    jitteramount = 1;
    jitterValuesY =  8*(rand(1))*jitteramount;
    scatter((arrivalframes11(ii)/3), 1+jitterValuesY,4, 'b', 'o');hold on;
end

for ii=1:(size(arrivalframes12))
    jitteramount = 1;
    jitterValuesY =  8*(rand(1))*jitteramount;
    scatter((arrivalframes12(ii)/3), 81+jitterValuesY,4, 'b', 'o');hold on;
end
...
for ii=1:(size(arrivalframes21))
    jitteramount = 1;
    jitterValuesY =  8*(rand(1))*jitteramount;
    scatter((arrivalframes21(ii)/3), 1+jitterValuesY,4, 'r', 'o');hold on;
end

for ii=1:(size(arrivalframes22))
    jitteramount = 1;
    jitterValuesY =  8*(rand(1))*jitteramount;
    scatter((arrivalframes22(ii)/3), 81+jitterValuesY,4, 'r', 'o');hold on;
end
xlabel('Time (sec)');
orient portrait;
