function [event_matrix] = mark_distance_events_mod032317(Tracks,reor_matrix, dist_min, dist_max,FrameRate)
...
odor_distance_matrix = track_field_to_matrix_mod032317(Tracks, 'odor_distance');

reor_matrix = reor_matrix';
reor_start_inds = find(reor_matrix); %these are starts of behavior of interest
track_start_reor_inds = reor_start_inds(mod(reor_start_inds,size(odor_distance_matrix,1))==1);
non_track_start_reor_inds = reor_start_inds(mod(reor_start_inds,size(odor_distance_matrix,1))~=1);

odor_distance_matrix(track_start_reor_inds) = NaN;%just in case any reorientations are the beginning of a track, don't replace with numbers from a previous track
odor_distance_matrix(non_track_start_reor_inds) = odor_distance_matrix(non_track_start_reor_inds - 1);%look to the distance right before the reorientation

%Creates empty matrix with NaNs
% time_matrix = NaN(size(odor_distance_matrix)); 
event_matrix = zeros(size(odor_distance_matrix));
% event_matrix(isnan(odor_distance_matrix_precise)==1) = NaN;

% Will mark a 1 if the odor_distance is not NaN and falls within the range
% specified between distance_min and distance_max

event_matrix(odor_distance_matrix>=dist_min & odor_distance_matrix<=dist_max) = 1;
